...
CirMgr::connect()
{
   for(int i = 0; i < _capacity; ++i)
   {
      ...
      // if input gate
      if(_totalList[i]->type() == PI_GATE || _totalList[i]->type() == CONST_GATE) continue;
      // if output gate
...
CirMgr::connect_gate(int& i, GateType gate_type)
{
   ...
   else
   {
      ...
      else // And gate (AIG Gate)
      {
         ...
         if(_totalList[in1] == 0)
         {
            // since there is no gate at in1, so create an undef gate at in1
            CirGate* p = new undefGate();
            _totalList[in1] = p;
...
CirMgr::genDFSlist()
{
   _dfsList.clear(); _pi.clear(); _po.clear();
...
{
   // first clear the previous result 
   _floatList.clear(); _unusedList.clear();
   // then fo through all totalList to check whether a gate is floating or unused
   for(int i = 0; i < _capacity; ++i)
   {
      if(_totalList[i]->type() == AIG_GATE)
      {
         if(_totalList[i]->_fanin.size() != 2) // not enough fanin
         {
            _floatList.insert(i);
         }
         else
         {
            vector<CirGateType>& fi = _totalList[i]->_fanin;
            for(int i = 0; i < fi.size(); ++i)
            {
               ...
               else if(fi[i].gate()->type() != AIG_GATE && fi[i].gate()->type() != PI_GATE)
               {
                  _floatList.insert(i);
               }
         ...
         if(_totalList[i]->_fanout.empty()) // no fanout
         {
            _unusedList.insert(i);
         }
         else 
         {
            vector<CirGateType>& fo = _totalList[i]->_fanout;
            for(int i = 0; i < fo.size(); ++i)
            {
               ...
               else if(fo[i].gate()->type() != AIG_GATE && fo[i].gate()->type() != PO_GATE)
               {
                  _unusedList.insert(i);
               }
            }
      ...
      // else if(_totalList[i]->type() == UNDEF_GATE)
      // {
      //    _unusedList.insert(i);
      // }
      // we do not consider UNDEF gate and CONST gate in this case
   }
}
...
{
   int A = 0; 
   // only AIG gate will change
   // cout << "_totalList size: " << _totalList.size() << endl;
   for(int i = 0; i < _capacity; ++i)
   {
      // cout << _totalList[i] << endl;
      if(_totalList[i] == 0) continue;
      // cout << _totalList[i]->type() << endl;
      if(_totalList[i]->type() == AIG_GATE) ++A;
   }
   // cout << A << endl;
   _A = A;
}
