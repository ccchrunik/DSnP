...
TaskMgr::add(size_t nMachines)
{
   ...
   for(int i = 0; i < nMachines; ++i)
   {
      // cout << i << endl;
      TaskNode newT;
      if(_taskHash.insert(newT))
      {
         cout << "Task node inserted: (" << newT.getName() << ", " << newT.getLoad() << ")" << endl;
         _taskHeap.insert(newT);
      }
      // _taskHash.insert(newT);
   }
}
...
{
   // TODO...
   // only had to check if hash's insert, since heap will definitely be right
   TaskNode t(s, l);
   if(!_taskHash.check(t)) 
   {
...
TaskMgr::assign(size_t l)
{
   ...
   n += l;
   _taskHash.update(n);
   _taskHeap.delMin(); _taskHeap.insert(n);
   return true;
}
